...
inline int ssd1306_get_font(uint8_t character)
{
  if (character >= 'A' && character <= 'Z') {
    return character - 'A' + 1; // Letras maiúsculas começam no índice 1
  }
  else if (character >= '0' && character <= '9') {
    return character - '0' + 27; // Números começam no índice 27
  }
  else if (character == '-') {
    return 37; // Índice do caractere '-' na tabela (ajuste de acordo com a sua tabela)
  }
  else {
    return 0; // Retorna vazio para caracteres não suportados
  }
}


// Desenha um único caractere com tamanho ajustável no display
void ssd1306_draw_char_with_size(uint8_t *ssd, int16_t x, int16_t y, uint8_t character, float size) {
    if (x > ssd1306_width - 8 * size || y > ssd1306_height - 8 * size) {
        return;
    }
    ...
    for (int i = 0; i < 8; i++) {
        uint8_t line = font[idx * 8 + i];
        for (int j = 0; j < 8; j++) {
            if (line & (1 << j)) {
                for (int sx = 0; sx < size; sx++) {
                    for (int sy = 0; sy < size; sy++) {
                        ssd1306_set_pixel(ssd, x + i * size + sx, y + j * size + sy, true);
                    }
                }
            }
        }
...
// Desenha uma string com tamanho ajustável, chamando a função de desenhar caractere várias vezes
void ssd1306_draw_string_with_size(uint8_t *ssd, int16_t x, int16_t y, char *string, float size) {
    if (x > ssd1306_width - 8 * size || y > ssd1306_height - 8 * size) {
        return;
    }

    while (*string) {
        ssd1306_draw_char_with_size(ssd, x, y, *string++, size);
        x += 8 * size;
    }
}
